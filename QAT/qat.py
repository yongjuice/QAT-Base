...
parser.add_argument('--fq', default=1, type=int,
                    help='Epoch to wait for fake-quantize activations. requires at least one epoch.')
...
def set_func_for_target_arch(arch):
    ...
    if arch == 'MLP':
        ...
        setattr(tools, 'fused_model_initializer', fused_mlp)
...
def main(args, data_loaders):
    args = argparse.Namespace(**vars(args_qat), **vars(args))
    print(vars(args))
    ...
    def specify_target_arch(arch, dataset):
        ...
        model_initializers = set_func_for_target_arch(arch)
    ...
    if args.mode == 'pre':
        ...
        _finetune(args, tools, data_loaders)
    elif args.mode == 'eval':
